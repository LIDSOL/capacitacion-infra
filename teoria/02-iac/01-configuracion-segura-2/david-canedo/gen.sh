...
SSH_PUBLIC_KEY="ssh-rsa   brasilia@Brasil"
...
initialize_environment() {
    ...
    if [ ! -d "$IMAGE_DIR" ]; then
        ...
        echo "Please create it (e.g., 'sudo mkdir -p $IMAGE_DIR && sudo chmod 775 $IMAGE_DIR && sudo chown $(whoami):libvirt $IMAGE_DIR' or similar)."
        echo "Alternatively, change the IMAGE_DIR variable in this script."
        exit 1
    fi

    BASE_IMAGE_PATH="$IMAGE_DIR/$BASE_IMAGE_FILENAME"
    if [ ! -f "$BASE_IMAGE_PATH" ]; then
        echo "Downloading Debian 12 cloud image to '$BASE_IMAGE_PATH'..."
        if [ "$(stat -c '%U' "$(dirname "$BASE_IMAGE_PATH")")" == "root" ] || [ ! -w "$(dirname "$BASE_IMAGE_PATH")" ]; then
            sudo wget -O "$BASE_IMAGE_PATH" "$IMAGE_URL"
        else
            wget -O "$BASE_IMAGE_PATH" "$IMAGE_URL"
        fi
        echo "Image downloaded."
    else
        echo "Base image '$BASE_IMAGE_PATH' already exists. Skipping download."
    fi

    if ! sudo virsh net-info "$LIBVIRT_NETWORK" | grep -q "Active: *yes"; then
        echo "Libvirt network '$LIBVIRT_NETWORK' is not active. Attempting to start it..."
        sudo virsh net-start "$LIBVIRT_NETWORK" || { echo >&2 "Failed to start network '$LIBVIRT_NETWORK'. Please check libvirt network configuration."; exit 1; }
        sudo virsh net-autostart "$LIBVIRT_NETWORK"
        echo "Network '$LIBVIRT_NETWORK' started and set to autostart."
    else
        echo "Libvirt network '$LIBVIRT_NETWORK' is active."
    fi
